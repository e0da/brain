import { times, handleEvents, sequence } from './util.mjs'
...
const update = state => {
  const { weems, width, height, exits } = state
  weems.forEach(weem => {
    handleEvents(weem, ({ segment, type }) => {
      switch (type) {
        ...
        case 'reset':
          ...
          del(weem, weems)
          // XXX This is a smell. Events should be consolidated
          exits.events.push({ type: 'reset' })
        default:
          move(weem, state)
      }
    })
